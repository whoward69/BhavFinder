...
namespace BhavFinder.DBPF.STR
{
	...
	public class StrLanguage : System.Collections.IComparer
	{
		...
		public string Name
		{
			get
			{
				string enumName = ((SimPe.Data.MetaData.Languages)lid).ToString();
				string s = SimPe.Localization.Manager.GetString( enumName );
				if (s != null) return s;
				return enumName;
			}
		}
		...
		{
			return "0x" + SimPe.Helper.HexString(lid) + " - " + this.Name;
		}
	...
	public class StrToken
	{
		...
		{
			get { return title; }
			set { if (title != value) { title = value; } }
		}
